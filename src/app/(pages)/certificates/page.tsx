...
export default function AllUserCertificatesPage() {
  ...
  const [receivedCertificates, setReceivedCertificates] = useState<
    Certificate[]
  >([]);
  const [createdCertificates, setCreatedCertificates] = useState<Certificate[]>(
    []
  );
          ...
          try {
            ...
            if (userIIN) {
              const received = response.data.filter(
                (cert: Certificate) =>
                  cert.recipientIIN === userIIN && cert.issuerIIN !== userIIN
              );
              const created = response.data.filter(
                (cert: Certificate) => cert.issuerIIN === userIIN
              );
          ...
          <>
            {certificateList.map((certificate) => (
              ...
              >
                ...
                <p
                  className="blur-[2.5px] truncate text-justify text-wrap w-full line-clamp-4"
                  style={{
                    WebkitBoxOrient: "vertical",
                    display: "-webkit-box",
                  }}
                >
                  {truncateText(certificate.certificateBody)}
                </p>
                <Button
                ...
                <p className="text-tertiary">
                  {title === "Received by you" ? "Issuer" : "Recipient"} IIN:{" "}
  ...
  return (
    <main className="flex flex-col items-center relative gap-12">
      <div className="mb-12 bg-[#4d4d4d10] backdrop-blur-xl border-[3px] border-borderdefault p-5 w-fit rounded-lg mt-56">
        <h1 className="text-5xl font-bold text-green">Your Certificates</h1>
      </div>
      ...
      {isLoading ? (
        ...
        <article className="flex justify-between gap-12 container">
          {certificates.length === 0 ? (
            ...
            <>
              {renderCertificateList(receivedCertificates, "Received by you")}
              {renderCertificateList(createdCertificates, "Created by you")}
            </>
          )}
        </article>
      )}
